...
void main()
{
    ...
    float light_intensity = smoothstep(0, toon_light_smooth_interpolation.x, NdotL);
    ...
    float specular_intensity = pow(NdotH * light_intensity, toon_glossiness.x * toon_glossiness.x);

    float specular_intensity_smooth = smoothstep(0.005, toon_specular_smooth.x, specular_intensity);
    ...
    rim_intensity = smoothstep(toon_rim_amount.x - toon_rim_smooth.x,
                               toon_rim_amount.x + toon_rim_smooth.x,
                               rim_intensity);
    vec4 rim = vec4(rim_intensity * toon_rim_color);

    fragColor = (light + toon_ambient_color + specular + rim) * toon_color * sample;
}